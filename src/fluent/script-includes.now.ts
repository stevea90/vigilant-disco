...
ATFBuilder.prototype = {
    ...
    build: function(stepDefs) {
        ...
        if (!atfActive) {
            return { suiteId: null, testId: null, error: 'ATF plugin is not active on this instance', steps: stepDefs };
        }

        try {
            var suiteId = this._createSuite();
            if (!suiteId) {
                return { suiteId: null, testId: null, error: 'Failed to create ATF suite (cross-scope write may be blocked)', steps: stepDefs };
            }

            var testId = this._createTest(suiteId);
            if (!testId) {
                return { suiteId: suiteId, testId: null, error: 'Failed to create ATF test', steps: stepDefs };
            }
            ...
            return { suiteId: suiteId, testId: testId, error: null, steps: null };
        } catch(e) {
            return { suiteId: null, testId: null, error: 'ATF table access failed: ' + e.message, steps: stepDefs };
        }
    },

    _createSuite: function() {
        try {
            var helper = new SDLC_AI_ATFHelper();
            return helper.createSuite('SDLC AI - Generated Suite ' + new GlideDateTime().getDisplayValue(), 'Auto-generated by SDLC AI Orchestrator');
        } catch(e) { /* global helper not available, fall through */ }

        var tables = ['sys_atf_test_suite', 'sys_atf_suite'];
    ...
    _createTest: function(suiteId) {
        try {
            var helper = new SDLC_AI_ATFHelper();
            return helper.createTest(suiteId, 'SDLC AI - Generated Test', 'Auto-generated by SDLC AI Orchestrator');
        } catch(e) {
            var gr = new GlideRecord('sys_atf_test');
            gr.initialize();
            gr.setValue('name', 'SDLC AI - Generated Test');
            gr.setValue('description', 'Auto-generated by SDLC AI Orchestrator');
            try { gr.setValue('sys_atf_suite', suiteId); } catch(fe) {}
            return gr.insert();
        }
    },
    ...
    _createStep: function(testId, stepDef, order) {
        ...
        var inputsJson = Object.keys(inputs).length > 0 ? JSON.stringify(inputs) : null;

        try {
            var helper = new SDLC_AI_ATFHelper();
            helper.createStep(testId, order * 100, configId, stepDef.description || '', inputsJson);
        } catch(e) {
            ...
            if (inputsJson) { try { gr.setValue('inputs', inputsJson); } catch(fe) {} }
            gr.insert();
        }
    },
    ...
    _resolveStepConfig: function(stepType, action) {
        try {
            var helper = new SDLC_AI_ATFHelper();
            var configId = helper.resolveStepConfig(stepType, action);
            if (configId) { return configId; }
        } catch(e) { /* fall through to local lookup */ }
...
AIOrchestrator.prototype = {
    ...
    createATFRecords: function() {
        ...
        try {
            ...
            if (atfResult.error) {
                if (atfResult.steps && atfResult.steps.length > 0) {
                    this._addTokens(result.tokens);
                    this.manifestGR.update();
                    this._setStatus('partial');
                    this._log('ATF records could not be created automatically (' + atfResult.error + '). ' + atfResult.steps.length + ' step(s) generated. Step definitions: ' + JSON.stringify(atfResult.steps));
                    gs.addInfoMessage('ATF step definitions generated but could not be written to ATF tables (cross-scope restriction). See the orchestration log in the Story Manifest for the step JSON.');
                    return { suiteId: null, testId: null };
                }
                this._setStatus('failed');
